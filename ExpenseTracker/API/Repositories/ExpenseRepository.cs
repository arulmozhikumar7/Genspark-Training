...
namespace ExpenseTrackerAPI.Repositories
{
    public class ExpenseRepository : IExpenseRepository
    {
        ...
        public async Task<(IEnumerable<Expense> Expenses, int TotalCount)> GetFilteredAsync(Guid userId, ExpenseQueryParameters parameters)
        {
            ...
            Guid? allCategoryId = await _context.Categories
                .Where(c => c.Name.ToLower() == "all")
                .Select(c => (Guid?)c.Id)
                .FirstOrDefaultAsync();

            if (parameters.CategoryId.HasValue && parameters.CategoryId.Value != allCategoryId)
            {
                query = query.Where(e => e.CategoryId == parameters.CategoryId);
            }
            ...
            {
                "amount" => parameters.SortDirection == "asc"
    ...
    public async Task<IEnumerable<Expense>> GetExpensesByDateRangeAsync(Guid userId, DateTime fromDate, DateTime toDate)
        {
            return await _context.Expenses
                .Include(e => e.Category)
                .Where(e => e.UserId == userId && !e.IsDeleted
